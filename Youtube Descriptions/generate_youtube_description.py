                ...
                for line in template_lines:
                    ...
                    if line_valid:
                        if is_boiler:
                            ...
                            if not is_numbered_piece:
                                if "," in modified_line:
                                    parts = modified_line.split(',', 1)
                                    title_part = parts[0].strip()
                                    rest_part = parts[1].strip()

                                    if "Organ Prelude-Concert" in title_part:
                                        chapter_lines.append(modified_line)
                                        last_performer = rest_part
                                        continue
                                    else:
                                        formatted = f"{title_part} - {rest_part}"
                                        chapter_lines.append(formatted)

                                        if "," in rest_part:
                                            last_performer = rest_part.rsplit(",", 1)[-1].strip()
                                        elif "-" in rest_part:
                                            last_performer = rest_part.rsplit("-", 1)[-1].strip()
                                        else:
                                            last_performer = None
                                ...
                                piece_performer = None

                                if "," in piece_content:
                                    p_parts = piece_content.rsplit(",", 1)
                                    piece_base = p_parts[0].strip()
                                    piece_performer = p_parts[1].strip()
                                else:
                                    piece_base = piece_content

                                formatted = piece_base
                                if piece_performer and piece_performer != last_performer:
                                    formatted += f", {piece_performer}"
                                    last_performer = piece_performer
